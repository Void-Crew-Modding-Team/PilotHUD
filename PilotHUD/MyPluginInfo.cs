#pragma warning disable CS1591
namespace PilotHUD
{
    //Auto-Generated File. Created by PreBuild.ps1
    public class MyPluginInfo
    {
        public const string PLUGIN_GUID = $"{PLUGIN_ORIGINAL_AUTHOR}.{PLUGIN_NAME}";
        public const string PLUGIN_NAME = "PilotHUD";
        public const string USERS_PLUGIN_NAME = "Pilot HUD";
        public const string PLUGIN_VERSION = "0.0.4";
        public const string PLUGIN_DESCRIPTION = "Displays the pilot HUD when the helm is powered.";
        public const string PLUGIN_ORIGINAL_AUTHOR = "18107";
        public const string PLUGIN_AUTHORS = "18107";
        public const string PLUGIN_THUNDERSTORE_ID = "VoidCrewModdingTeam/Pilot_HUD";
    }
}
#pragma warning restore CS1591
